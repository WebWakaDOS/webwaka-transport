 ...
 *
 * QA-TRA-1: Matching engine queries `trns_active_drivers` table and returns
 * the 5 nearest trns_drivers using the Haversine formula.
 *
 * Covers:
 *  - Haversine distance accuracy (known coordinates)
 *  - Bounding-box pre-filter (trns_drivers outside radius excluded)
 ...
 *  - Empty result when no trns_drivers available
...
describe('findNearestDrivers', () => {
  ...
  it('returns up to 5 trns_drivers (QA-TRA-1 — MAX_RESULTS = 5)', async () => {
    // Provide 8 available trns_drivers
  ...
  it('returns fewer than 5 when fewer trns_drivers are available', async () => {
    const rows = [
  ...
  it('sorts trns_drivers by distance ascending (nearest first)', async () => {
    const now = Date.now();
    const rows = [
  ...
  it('returns empty array when no trns_drivers in DB', async () => {
    const db = makeMockDb([]);
    const result = await findNearestDrivers(db, rider, undefined, 50);
...
describe('upsertDriverLocation', () => {
  it('calls db.prepare with correct SQL upsert pattern', async () => {
    ...
    expect(sql).toContain('INSERT INTO trns_active_drivers');
