 ...
 * Tests all Hono API trns_routes using in-memory mock D1
 */
...
function createMockDB() {
  const tables: Record<string, any[]> = {
    trns_trips: [], trns_seats: [], trns_operators: [], trns_routes: [], trns_vehicles: [],
    trns_agents: [], trns_sales_transactions: [], trns_receipts: [], trns_customers: [],
    trns_bookings: [], trns_trip_state_transitions: [], trns_sync_mutations: [], trns_drivers: [],
    trns_platform_events: [], trns_fare_rules: [], trns_trip_parcels: [],
  };
...
describe('TRN-1: Seat Inventory API', () => {
  ...
  it('GET /trns_trips returns empty list initially', async () => {
    const res = await seatInventoryRouter.request('/trns_trips', {}, makeEnv(db));
  ...
  it('POST /trns_trips creates a trip and trns_seats', async () => {
    const res = await seatInventoryRouter.request('/trns_trips', {
      method: 'POST',
  ...
  it('POST /trns_trips returns 400 if required fields missing', async () => {
    const res = await seatInventoryRouter.request('/trns_trips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('GET /trns_trips/:id/availability returns seat map', async () => {
    // Pre-populate trip and trns_seats
    db._tables.trns_trips.push({ id: 'trp_1', state: 'scheduled', deleted_at: null });
    db._tables.trns_seats.push({
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/availability', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  ...
  it('POST /trns_trips/:id/reserve returns 400 if seat_id missing', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/reserve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:id/reserve returns 404 for unknown seat', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/reserve', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:id/release returns 400 when token is missing (SEC-006)', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/release', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:id/release returns 403 when wrong token supplied (SEC-006)', async () => {
    db._tables.trns_seats.push({
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_sec/release', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:id/release releases seat with correct token (SEC-006)', async () => {
    db._tables.trns_seats.push({
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_sec2/release', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch returns 400 when seat_ids missing', async () => {
    db._tables.trns_trips.push({ id: 'trp_rb', operator_id: 'opr_1', deleted_at: null });
    const res = await seatInventoryRouter.request('/trns_trips/trp_rb/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch returns 400 when seat_ids is empty', async () => {
    db._tables.trns_trips.push({ id: 'trp_rb2', operator_id: 'opr_1', deleted_at: null });
    const res = await seatInventoryRouter.request('/trns_trips/trp_rb2/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch returns 400 when more than 10 trns_seats requested', async () => {
    db._tables.trns_trips.push({ id: 'trp_rb3', operator_id: 'opr_1', deleted_at: null });
    const res = await seatInventoryRouter.request('/trns_trips/trp_rb3/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    expect(res.status).toBe(400);
    const body = await res.json() as any;
    expect(body.error).toMatch(/10 trns_seats/i);
  });

  it('POST /trns_trips/:tripId/reserve-batch returns 404 for unknown trip (without DO binding)', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_ghost/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch succeeds via DO stub returning 200', async () => {
    db._tables.trns_trips.push({ id: 'trp_do1', operator_id: 'opr_1', deleted_at: null });
    db._tables.trns_seats.push({ id: 'sd1', trip_id: 'trp_do1', status: 'available' });
    db._tables.trns_seats.push({ id: 'sd2', trip_id: 'trp_do1', status: 'available' });
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_do1/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch forwards 409 from DO stub (seat unavailable)', async () => {
    db._tables.trns_trips.push({ id: 'trp_do2', operator_id: 'opr_1', deleted_at: null });
    db._tables.trns_seats.push({ id: 'sd3', trip_id: 'trp_do2', status: 'available' });
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_do2/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch forwards concurrent_conflict 409 with conflicted_seats from DO stub (BUG-3)', async () => {
    db._tables.trns_trips.push({ id: 'trp_do3', operator_id: 'opr_1', deleted_at: null });
    db._tables.trns_seats.push({ id: 'sd4', trip_id: 'trp_do3', status: 'available' });
    db._tables.trns_seats.push({ id: 'sd5', trip_id: 'trp_do3', status: 'available' });
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_do3/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch returns idempotent cached response on replay', async () => {
    db._tables.trns_trips.push({ id: 'trp_idem', operator_id: 'opr_1', deleted_at: null });
    db._tables.trns_seats.push({ id: 'si1', trip_id: 'trp_idem', status: 'available' });
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_idem/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_trips/:tripId/reserve-batch works via fallback D1 path when no DO binding', async () => {
    db._tables.trns_trips.push({ id: 'trp_fallback', operator_id: 'opr_1', deleted_at: null });
    db._tables.trns_seats.push({ id: 'sf1', trip_id: 'trp_fallback', status: 'available', version: 0 });

    const res = await seatInventoryRouter.request('/trns_trips/trp_fallback/reserve-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
describe('TRN-2: Agent Sales API', () => {
  ...
  it('GET /trns_agents returns empty list', async () => {
    const res = await agentSalesRouter.request('/trns_agents', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  });

  it('POST /trns_agents creates an agent', async () => {
    const res = await agentSalesRouter.request('/trns_agents', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_agents returns 400 if phone missing', async () => {
    const res = await agentSalesRouter.request('/trns_agents', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /transactions creates a sale in kobo', async () => {
    // Pre-populate trns_seats so the availability pre-check passes
    db._tables.trns_seats.push({ id: 's1', trip_id: 'trp_1', status: 'available' });
    db._tables.trns_seats.push({ id: 's2', trip_id: 'trp_1', status: 'available' });
...
describe('TRN-3: Booking Portal API', () => {
  ...
  it('GET /trns_routes returns empty list', async () => {
    const res = await bookingPortalRouter.request('/trns_routes', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  });

  it('POST /trns_customers requires NDPR consent', async () => {
    const res = await bookingPortalRouter.request('/trns_customers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_customers creates customer with NDPR consent', async () => {
    const res = await bookingPortalRouter.request('/trns_customers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_bookings requires NDPR consent', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('POST /trns_bookings returns 400 if seat_ids empty', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('GET /trns_bookings returns empty list', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  });

  it('GET /trns_bookings/:id returns 404 for unknown booking', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/unknown', {}, makeEnv(db));
    expect(res.status).toBe(404);
  });
});
...
describe('TRN-4: Operator Management API', () => {
  ...
  it('GET /trns_operators returns empty list', async () => {
    const res = await operatorManagementRouter.request('/trns_operators', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  });

  it('POST /trns_operators creates an operator', async () => {
    const res = await operatorManagementRouter.request('/trns_operators', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_operators returns 400 if code missing', async () => {
    const res = await operatorManagementRouter.request('/trns_operators', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_routes creates a route with kobo fare', async () => {
    const res = await operatorManagementRouter.request('/trns_routes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('POST /trns_routes rejects non-integer fare', async () => {
    const res = await operatorManagementRouter.request('/trns_routes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('POST /trns_vehicles registers a vehicle', async () => {
    const res = await operatorManagementRouter.request('/trns_vehicles', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('POST /trns_trips/:id/transition enforces valid state machine', async () => {
    // Pre-populate trip in 'completed' state
    db._tables.trns_trips.push({
      id: 'trp_done', state: 'completed', deleted_at: null,
      operator_id: 'opr_1', route_id: 'rte_1', vehicle_id: 'veh_1',
      departure_time: Date.now(), created_at: Date.now(), updated_at: Date.now(),
    });
    const res = await operatorManagementRouter.request('/trns_trips/trp_done/transition', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('GET /dashboard returns trip state counts', async () => {
    ...
    const body = await res.json() as any;
    expect(body.success).toBe(true);
    expect(body.data.trns_trips).toHaveProperty('scheduled');
    expect(body.data.trns_trips).toHaveProperty('in_transit');
  });
});
...
describe('Phase 2: PATCH /trns_trips/:tripId/trns_seats/:seatId — Seat Update (TRN-1)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 400 for invalid status value', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/trns_seats/s1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 for unknown seat', async () => {
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/trns_seats/s_unknown', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('accepts valid statuses: available, reserved, confirmed, blocked', async () => {
    ...
    const res = await seatInventoryRouter.request('/trns_trips/trp_1/trns_seats/trp_1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'blocked' }),
    }, makeEnv(db));
...
describe('Phase 2: PATCH /trns_bookings/:id — Booking Update (TRN-3)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 404 for unknown booking', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_unknown', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 409 for cancelled booking', async () => {
    ...
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ payment_reference: 'pay_new_ref' }),
    }, makeEnv(db));
  ...
  it('returns 422 when attempting to set status to confirmed via PATCH', async () => {
    ...
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_2', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('updates payment_reference successfully', async () => {
    ...
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_3', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
describe('Phase 2: PATCH /trns_trips/:id — Trip Update (TRN-4)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 404 for unknown trip', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_unknown', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('updates trip vehicle and departure time', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_trips/trp_upd', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
describe('Phase 2: DELETE /trns_trips/:id — Soft Delete Trip (TRN-4)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 404 for unknown trip', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_gone', {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(404);
  });
  ...
  it('soft-deletes a scheduled trip', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_trips/trp_del', {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('returns 409 for a boarding trip', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_trips/trp_boarding', {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(409);
    const body = await res.json() as any;
  ...
  it('returns 409 for an in_transit trip', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_trips/trp_transit', {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(409);
  });
...
describe('Phase 2: PATCH /trns_routes/:id — Route Update (TRN-4)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 404 for unknown route', async () => {
    const res = await operatorManagementRouter.request('/trns_routes/rte_unknown', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('updates route base_fare', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_routes/rte_2', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('updates route status to inactive', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_routes/rte_3', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
describe('Phase 2: PATCH /trns_vehicles/:id — Vehicle Update (TRN-4)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('returns 404 for unknown vehicle', async () => {
    const res = await operatorManagementRouter.request('/trns_vehicles/veh_unknown', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('updates vehicle status to maintenance', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_vehicles/veh_1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'maintenance' }),
    }, makeEnv(db));
  ...
  it('updates vehicle model and total_seats', async () => {
    ...
    const res = await operatorManagementRouter.request('/trns_vehicles/veh_2', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
describe('Phase 7: POST /trns_trips — Trip Creation (TRN-4)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_routes.push({
  ...
  it('creates a trip and batch-inserts trns_seats', async () => {
    const dep = Date.now() + 3600_000;
    const res = await operatorManagementRouter.request('/trns_trips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const trns_seats = db._tables.trns_seats.filter((s: any) => s.trip_id === body.data.id);
    expect(trns_seats.length).toBe(18);
    expect(trns_seats[0].seat_number).toBe('01');
    expect(trns_seats[17].seat_number).toBe('18');
  });
  ...
  it('accepts a total_seats override', async () => {
    ...
    const trns_seats = db._tables.trns_seats.filter((s: any) => s.trip_id === body.data.id);
    expect(trns_seats.length).toBe(5);
  });
...
describe('Phase 7: PATCH /trns_bookings/:id/cancel — Booking Cancellation (TRN-3)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_bookings.push({
  ...
  it('cancels a pending booking successfully', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_p7/cancel', {
      method: 'PATCH',
    }, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('returns 409 when trying to cancel an already-cancelled booking', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_p7_already_cancelled/cancel', {
      method: 'PATCH',
    }, makeEnv(db));
    expect(res.status).toBe(409);
    const body = await res.json() as any;
  ...
  it('returns 404 for unknown booking', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_ghost/cancel', {
      method: 'PATCH',
    }, makeEnv(db));
    expect(res.status).toBe(404);
  });
...
describe('Phase 12: POST /trns_trips/:id/copy — Duplicate Trip', () => {
  let db: any;
  const now = Date.now();
  ...
  it('creates a copy with a new departure time and returns 201', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_src1/copy', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 for unknown source trip', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_ghost/copy', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ departure_time: futureMs }),
    }, makeEnv(db));
...
// ============================================================
// Phase 11 — Super Admin: PATCH /trns_operators/:id Tests
// ============================================================

describe('Phase 11: PATCH /trns_operators/:id — Update Operator (SUPER_ADMIN)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_operators.push({
  ...
  it('suspends an active operator', async () => {
    const res = await operatorManagementRouter.request('/trns_operators/opr_upd1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 for unknown operator id', async () => {
    const res = await operatorManagementRouter.request('/trns_operators/opr_ghost', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'suspended' }),
    }, makeEnv(db));
...
describe('Phase 10: PATCH /trns_agents/:id — Update Agent (TRN-2)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_agents.push({
      id: 'agt_upd1', operator_id: 'opr_1', name: 'Chidi Obi', phone: '08011112222',
      email: null, role: 'agent', trns_bus_parks: '[]', status: 'active',
      created_at: Date.now(), updated_at: Date.now(), deleted_at: null,
    });
  });

  it('suspends an active agent', async () => {
    const res = await agentSalesRouter.request('/trns_agents/agt_upd1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'suspended' }),
    }, makeEnv(db));
  ...
  it('updates trns_bus_parks array', async () => {
    const res = await agentSalesRouter.request('/trns_agents/agt_upd1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ trns_bus_parks: ['park_a', 'park_b'] }),
    }, makeEnv(db));
    expect(res.status).toBe(200);
  });

  it('returns 404 for unknown agent', async () => {
    const res = await agentSalesRouter.request('/trns_agents/agt_ghost', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'suspended' }),
    }, makeEnv(db));
...
describe('Phase 10: GET /reports/revenue — Revenue Report (TRN-4)', () => {
  ...
  beforeEach(() => {
    ...
    db._tables.trns_sales_transactions.push({
  ...
  it('returns booking revenue from paid trns_bookings', async () => {
  ...
  it('returns zero revenue when no data exists', async () => {
    db._tables.trns_bookings = [];
    db._tables.trns_sales_transactions = [];
    const from = now - 7200_000;
...
describe('Phase 9: POST /trns_drivers — Create Driver (TRN-4)', () => {
  let db: any;
  beforeEach(() => { db = createMockDB(); });

  it('creates a driver and returns 201', async () => {
    const res = await operatorManagementRouter.request('/trns_drivers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    expect(db._tables.trns_drivers).toHaveLength(1);
  });
...
describe('Phase 9: GET /trns_drivers — List Drivers (TRN-4)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_drivers.push(
  ...
  it('returns all non-deleted trns_drivers for operator', async () => {
    const res = await operatorManagementRouter.request('/trns_drivers?operator_id=opr_1', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
...
describe('Phase 9: PATCH /trns_drivers/:id — Update Driver (TRN-4)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_drivers.push({
  ...
  it('updates driver status to suspended', async () => {
    const res = await operatorManagementRouter.request('/trns_drivers/drv_upd1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'suspended' }),
    }, makeEnv(db));
  ...
  it('returns 404 for unknown driver', async () => {
    const res = await operatorManagementRouter.request('/trns_drivers/drv_ghost', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'suspended' }),
    }, makeEnv(db));
...
describe('Phase 9: PATCH /trns_trips/:id with driver_id — Assign Driver (TRN-4)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_trips.push({
  ...
  it('assigns driver_id to an existing trip', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_d1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns 404 when trip does not exist', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_ghost_d', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ driver_id: 'drv_a1' }),
    }, makeEnv(db));
...
describe('Phase 9: GET /trns_trips/:id/manifest — driver field present', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_trips.push({
  ...
  it('manifest trip includes driver name and phone', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_dm1/manifest', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('manifest trip driver is null when no driver assigned', async () => {
    db._tables.trns_trips[0].driver_id = null;
    const res = await operatorManagementRouter.request('/trns_trips/trp_dm1/manifest', {}, makeEnv(db));
    const body = await res.json() as any;
...
describe('Phase 8: GET /trns_trips/:id/manifest — Trip Manifest (TRN-4)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_trips.push({
    ...
    db._tables.trns_customers.push({
  ...
  it('returns manifest with trip summary and passengers', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_m1/manifest', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  ...
  it('includes confirmed_revenue_kobo for paid trns_bookings', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_m1/manifest', {}, makeEnv(db));
    const body = await res.json() as any;
    expect(body.data.summary.confirmed_revenue_kobo).toBe(1000000);
  });

  it('returns empty passengers when no trns_bookings exist', async () => {
    db._tables.trns_bookings = [];
    const res = await operatorManagementRouter.request('/trns_trips/trp_m1/manifest', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('returns 404 for unknown trip', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_ghost/manifest', {}, makeEnv(db));
    expect(res.status).toBe(404);
    const body = await res.json() as any;
    expect(body.error).toMatch(/trip not found/i);
  });

  it('load_factor reflects trns_seats vs trns_bookings ratio', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_m1/manifest', {}, makeEnv(db));
    const body = await res.json() as any;
    expect(body.data.summary.load_factor).toBe(33); // 1/3 trns_seats booked = 33%
  });
});
...
describe('T-TRN-02: Manifest — next_of_kin fields in JSON response', () => {
  ...
  it('manifest JSON includes next_of_kin_name and next_of_kin_phone for trns_bookings', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_nok1/manifest', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  ...
  it('manifest JSON next_of_kin fields are null when not provided', async () => {
    db._tables.trns_bookings[0].next_of_kin_name = null;
    db._tables.trns_bookings[0].next_of_kin_phone = null;
    const res = await operatorManagementRouter.request('/trns_trips/trp_nok1/manifest', {}, makeEnv(db));
    const body = await res.json() as any;
    const p = body.data.passengers[0];
...
describe('T-TRN-02: Manifest — agent_sales includes next_of_kin and passenger_id_hash', () => {
  ...
  it('manifest JSON agent_sales includes next_of_kin and passenger_id_hash', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_agtnok/manifest', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('manifest JSON agent_sales next_of_kin fields are null when not stored', async () => {
    db._tables.trns_sales_transactions[0].next_of_kin_name = null;
    db._tables.trns_sales_transactions[0].next_of_kin_phone = null;
    db._tables.trns_sales_transactions[0].passenger_id_hash = null;
    const res = await operatorManagementRouter.request('/trns_trips/trp_agtnok/manifest', {}, makeEnv(db));
    const body = await res.json() as any;
    const a = body.data.agent_sales[0];
...
describe('T-TRN-02: POST /transactions — stores next_of_kin_name and next_of_kin_phone', () => {
  ...
  it('stores next_of_kin_name and next_of_kin_phone when provided', async () => {
    ...
    const body = await res.json() as any;
    expect(body.success).toBe(true);
    const stored = db._tables.trns_sales_transactions.find((t: any) => t.agent_id === 'agt_nok2');
...
describe('T-TRN-02: GET /trns_trips/:id/manifest CSV — includes both trns_bookings and agent sales (FRSC compliance)', () => {
  let db: any;
  const now = Date.now();
  beforeEach(() => {
    db = createMockDB();
  ...
  it('CSV includes online booking row with Source=Online', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_csv1/manifest', {
...
describe('Phase 8: GET /trns_bookings/:id — Booking Detail (TRN-3)', () => {
  let db: any;
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_bookings.push({
  ...
  it('returns booking detail with seat_ids and passenger_names', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_detail1', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
  ...
  it('returns 404 for unknown booking', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings/bkg_ghost', {}, makeEnv(db));
    expect(res.status).toBe(404);
  });
});
...
describe('Phase 2: Pagination meta in list responses', () => {
  ...
  it('GET /trns_trips includes meta.limit and meta.offset (TRN-1)', async () => {
    const res = await seatInventoryRouter.request('/trns_trips?limit=10&offset=0', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('GET /trns_agents includes meta.limit (TRN-2)', async () => {
    const res = await agentSalesRouter.request('/trns_agents?limit=5', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('GET /trns_bookings includes pagination meta (TRN-3)', async () => {
    const res = await bookingPortalRouter.request('/trns_bookings?limit=25&offset=50', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('GET /trns_operators includes pagination meta (TRN-4)', async () => {
    const res = await operatorManagementRouter.request('/trns_operators?limit=20', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('GET /trns_routes includes pagination meta (TRN-4)', async () => {
    const res = await operatorManagementRouter.request('/trns_routes?limit=10&offset=0', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
    expect(body.meta.limit).toBe(10);
  ...
  it('GET /trns_vehicles includes pagination meta (TRN-4)', async () => {
    const res = await operatorManagementRouter.request('/trns_vehicles?limit=50', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('GET /trns_trips includes pagination meta (TRN-4)', async () => {
    const res = await operatorManagementRouter.request('/trns_trips?limit=100&offset=200', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.meta).toBeDefined();
  ...
  it('clamps limit to max 200', async () => {
    const res = await seatInventoryRouter.request('/trns_trips?limit=999', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
...
describe('Phase 2: Input validation and error handling', () => {
  ...
  it('POST /trns_trips requires total_seats to be a positive integer (TRN-1)', async () => {
    const res = await seatInventoryRouter.request('/trns_trips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it('POST /trns_trips/:id/transition returns 400 if to_state missing (TRN-4)', async () => {
    db._tables.trns_trips.push({
      id: 'trp_x', state: 'scheduled', deleted_at: null,
      operator_id: 'opr_1', route_id: 'rte_1', vehicle_id: 'veh_1',
      departure_time: Date.now(), created_at: Date.now(), updated_at: Date.now(),
    });
    const res = await operatorManagementRouter.request('/trns_trips/trp_x/transition', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    }, makeEnv(db));
  ...
  it('PATCH /trns_trips/:id/location rejects string coordinates (TRN-4)', async () => {
    const res = await operatorManagementRouter.request('/trns_trips/trp_1/location', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_customers returns 400 for missing phone (TRN-3)', async () => {
    const res = await bookingPortalRouter.request('/trns_customers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('GET /trns_receipts/:id returns 404 for unknown receipt (TRN-2)', async () => {
    const res = await agentSalesRouter.request('/trns_receipts/rct_unknown', {}, makeEnv(db));
    expect(res.status).toBe(404);
    const body = await res.json() as any;
    expect(body.success).toBe(false);
  });
...
describe('Paystack Payments API', () => {
  ...
  it('POST /verify auto-confirms booking in dev mode', async () => {
    ...
    const booking = db._tables.trns_bookings.find((b: any) => b.id === 'bkg_dev2');
    expect(booking?.status).toBe('confirmed');
    const seat = db._tables.trns_seats.find((s: any) => s.id === 'seat_2');
...
describe('C-008: PATCH /users/:id/role — Admin Promotion API', () => {
  ...
  beforeEach(() => {
    ...
    db._tables.trns_agents.push({
      ...
      email: null, role: 'STAFF', trns_bus_parks: '[]', status: 'active',
      created_at: Date.now(), updated_at: Date.now(), deleted_at: null,
    });
    db._tables.trns_customers.push({
  ...
  it('returns 404 when user does not exist in trns_agents or trns_customers', async () => {
...
describe('T-TRN-03: Fare Rules API', () => {
  ...
  beforeEach(() => {
    db = createMockDB();
    db._tables.trns_operators.push({ id: OPERATOR_ID, name: 'FareOp', code: 'FARE', status: 'active', deleted_at: null, created_at: 1, updated_at: 1 });
    db._tables.trns_routes.push({
  ...
  it('GET /trns_routes/:id/fare-rules returns empty array when no rules', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules`, {
      method: 'GET',
    }, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('POST /trns_routes/:id/fare-rules creates an always rule', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_routes/:id/fare-rules creates a surge_period rule', async () => {
  ...
  it('POST /trns_routes/:id/fare-rules rejects invalid rule_type', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('POST /trns_routes/:id/fare-rules rejects multiplier > 10', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('GET /trns_routes/:id/fare-rules returns rules after creation', async () => {
    // Seed a rule
    db._tables.trns_fare_rules.push({
  ...
  it('DELETE /trns_routes/:id/fare-rules/:ruleId soft-deletes the rule', async () => {
    db._tables.trns_fare_rules.push({
    ...
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules/far_del1`, {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    ...
    const rule = db._tables.trns_fare_rules.find((r: any) => r.id === 'far_del1');
    expect(rule.is_active).toBe(0);
    expect(rule.deleted_at).toBeGreaterThan(0);
  });

  it('DELETE /trns_routes/:id/fare-rules/:ruleId returns 404 for unknown rule', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules/far_ghost`, {
      method: 'DELETE',
    }, makeEnv(db));
    expect(res.status).toBe(404);
  });

  it('PUT /trns_routes/:id/fare-rules/:ruleId updates rule name and multiplier', async () => {
    db._tables.trns_fare_rules.push({
    ...
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/fare-rules/far_upd1`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('GET /trns_routes/:id/effective-fare returns 200 for own operator route', async () => {
    const res = await operatorManagementRouter.request(`/trns_routes/${ROUTE_ID}/effective-fare`, {
      method: 'GET',
    }, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it('GET /trns_routes/:id/effective-fare returns 404 for unknown route', async () => {
    const res = await operatorManagementRouter.request('/trns_routes/rte_ghost/effective-fare', {
      method: 'GET',
    }, makeEnv(db));
...
describe('T-TRN-03: Booking Fare Lock Integration', () => {
  ...
  beforeEach(() => {
    ...
    db._tables.trns_operators.push({ id: 'opr_fl1', name: 'FareLockOp', code: 'FLK', status: 'active', deleted_at: null, created_at: now, updated_at: now });
    db._tables.trns_routes.push({
  ...
  // Each test seeds its own trns_seats to avoid mock-DB all() cross-contamination
  // (all() returns rows matching ANY param — two trns_seats sharing a trip_id would both match)
...
describe('T-TRN-04: Paystack Inline — webhook verification and payment.successful event', () => {
  ...
  it('POST /webhooks/paystack confirms booking and emits payment.successful on charge.success', async () => {
    ...
    const booking = db._tables.trns_bookings.find((b: any) => b.id === bookingId);
    expect(booking?.status).toBe('confirmed');
    expect(booking?.payment_status).toBe('completed');
    expect(booking?.payment_provider).toBe('paystack');

    const seat = db._tables.trns_seats.find((s: any) => s.id === 'seat_ps1');
    expect(seat?.status).toBe('confirmed');

    const events = db._tables.trns_platform_events.filter(
  ...
  it('POST /webhooks/paystack is idempotent — skips already-confirmed trns_bookings', async () => {
    const db = createMockDB();
  ...
  it('POST /webhooks/flutterwave confirms booking and emits payment.successful on charge.completed', async () => {
    ...
    const seat = db._tables.trns_seats.find((s: any) => s.id === 'seat_fw1');
    expect(seat?.status).toBe('confirmed');
...
describe('T-TRN-04: drainEventBus — payment.successful consumer', () => {
  ...
  it('processes payment.successful event without error (success path — retry_count stays null)', async () => {
    ...
    // Booking with denormalized JOIN fields (mock first() returns the full trns_bookings row;
    ...
    db._tables.trns_platform_events.push({
      id: evtId,
    ...
    const evt = db._tables.trns_platform_events.find((e: any) => e.id === evtId);
...
// logisticsRouter: POST/GET/DELETE /api/logistics/trns_trips/:tripId/parcels
...
describe('T-TRN-05: Logistics — cargo parcel endpoints', () => {
  ...
  // ── T05-01 ──────────────────────────────────────────────────
  it('POST /trns_trips/:tripId/parcels returns 400 when tracking_ref missing', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_cargo_01/parcels', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // ── T05-02 ──────────────────────────────────────────────────
  it('POST /trns_trips/:tripId/parcels returns 404 for unknown trip', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_ghost_99/parcels', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // ── T05-03 ──────────────────────────────────────────────────
  it('POST /trns_trips/:tripId/parcels returns 409 when trip is completed', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_done_01/parcels', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // ── T05-04 ──────────────────────────────────────────────────
  it('POST /trns_trips/:tripId/parcels loads a parcel successfully and emits event', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_cargo_01/parcels', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const savedParcels = db._tables.trns_trip_parcels;
    ...
    // Event emitted
    const events = db._tables.trns_platform_events.filter((e: any) => e.event_type === 'trip.cargo_loaded');
  ...
  // ── T05-05 ──────────────────────────────────────────────────
  it('POST /trns_trips/:tripId/parcels normalises tracking_ref to uppercase', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_cargo_01/parcels', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // ── T05-06 ──────────────────────────────────────────────────
  it('GET /trns_trips/:tripId/parcels returns 404 for unknown trip', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_ghost_list/parcels', {}, makeEnv(db));
    expect(res.status).toBe(404);
    const body = await res.json() as any;
    expect(body.success).toBe(false);
  });
  ...
  // ── T05-07 ──────────────────────────────────────────────────
  it('GET /trns_trips/:tripId/parcels returns empty list when none loaded', async () => {
    const res = await logisticsRouter.request('/trns_trips/trp_cargo_01/parcels', {}, makeEnv(db));
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.success).toBe(true);
    expect(Array.isArray(body.data)).toBe(true);
  ...
  // ── T05-08 ──────────────────────────────────────────────────
  it('GET /trns_trips/:tripId/parcels returns loaded parcels', async () => {
    // Pre-populate two parcels
    const now = Date.now();
    db._tables.trns_trip_parcels.push({
  ...
  // ── T05-09 ──────────────────────────────────────────────────
  it('DELETE /trns_trips/:tripId/parcels/:trackingRef returns 404 for unknown trip', async () => {
    const res = await logisticsRouter.request(
      '/trns_trips/trp_ghost_del/parcels/PKG-001',
      { method: 'DELETE' },
      makeEnv(db)
    );
    expect(res.status).toBe(404);
  ...
  // ── T05-10 ──────────────────────────────────────────────────
  it('DELETE /trns_trips/:tripId/parcels/:trackingRef removes an on_board parcel and emits event', async () => {
    // Mock first() for trns_trip_parcels matches by last param = trackingRef
    ...
    const res = await logisticsRouter.request(
      '/trns_trips/trp_cargo_01/parcels/PKG-DEL',
      { method: 'DELETE' },
      makeEnv(db)
    );
    expect(res.status).toBe(200);
    ...
    // DB updated
    const parcel = db._tables.trns_trip_parcels.find((p: any) => p.id === 'PKG-DEL');
    expect(parcel.status).toBe('removed');

    // Event emitted
    const events = db._tables.trns_platform_events.filter((e: any) => e.event_type === 'trip.cargo_unloaded');
    expect(events.length).toBe(1);
    const payload = JSON.parse(events[0].payload);
  ...
  it('DELETE returns 409 when parcel is already delivered (not on_board)', async () => {
    ...
    const res = await logisticsRouter.request(
      '/trns_trips/trp_cargo_01/parcels/PKG-DONE',
      { method: 'DELETE' },
      makeEnv(db)
    );
    expect(res.status).toBe(409);
  ...
  it('drainEventBus forwards trip.cargo_loaded to logistics consumer URL', async () => {
    ...
    try {
      ...
      const evt = db._tables.trns_platform_events.find((e: any) => e.id === 'evt-cargo-loaded-01');
      expect(evt.status).not.toBe('pending');
    } finally {
  ...
  it('drainEventBus forwards trip.cargo_unloaded to logistics consumer URL', async () => {
    ...
    try {
      ...
      const evt = db._tables.trns_platform_events.find((e: any) => e.id === 'evt-cargo-unloaded-01');
      expect(evt.status).not.toBe('pending');
    } finally {
      globalThis.fetch = origFetch;
    }
  ...
  it('emitCargoUnloadedOnTripEnd — trip_cancelled marks on-board parcels as removed and emits event', async () => {
    ...
    const p1 = db._tables.trns_trip_parcels.find((p: any) => p.id === 'prc_can1');
    const p2 = db._tables.trns_trip_parcels.find((p: any) => p.id === 'prc_can2');
  ...
  it('emitCargoUnloadedOnTripEnd — trip_completed marks on-board parcels as delivered', async () => {
    ...
    const p = db._tables.trns_trip_parcels.find((p: any) => p.id === 'prc_comp1');
